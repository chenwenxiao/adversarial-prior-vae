...
def q_net(x, posterior_flow, observed=None, n_z=None):
    ...
    with arg_scope([spt.layers.resnet_conv2d_block],
        ...
        h_x = tf.to_float(x)
        h_x = spt.layers.resnet_conv2d_block(h_x, 64, kernel_size=5, strides=2 if config.dataset == 'celebA' else 1,
                                             scope='level_0')  # output: (28, 28, 16)
        h_x = spt.layers.resnet_conv2d_block(h_x, 64, scope='level_1')  # output: (14, 14, 32)
        h_x = spt.layers.resnet_conv2d_block(h_x, 96, strides=2, scope='level_2')  # output: (14, 14, 32)
        h_x = spt.layers.resnet_conv2d_block(h_x, 96, scope='level_4')  # output: (14, 14, 32)
...
def G_theta(z, return_std=False):
    ...
    with arg_scope([spt.layers.resnet_deconv2d_block],
                   ...
                   normalizer_fn=normalizer_fn,
                   kernel_regularizer=spt.layers.l2_regularizer(config.l2_reg)):
        h_z = spt.layers.dense(z, 128 * 7 * 7, scope='level_0',
                               normalizer_fn=None)
        ...
        h_z = spt.layers.resnet_deconv2d_block(h_z, 96, strides=2, scope='level_3')  # output: (7, 7, 64)
        h_z = spt.layers.resnet_deconv2d_block(h_z, 96, scope='level_5')  # output: (7, 7, 64)
        ...
        h_z = spt.layers.resnet_deconv2d_block(h_z, 64, kernel_size=5, strides=2 if config.dataset == 'celebA' else 1,
...
def G_omega(z, output_dim):
    ...
    with arg_scope([spt.layers.resnet_deconv2d_block],
        ...
        h_z = spt.layers.resnet_deconv2d_block(h_z, 128, scope='level_3')  # output: (14, 14, 32)
...
def D_psi(x, y=None):
    ...
    with arg_scope([spt.layers.resnet_conv2d_block],
                   ...
                   kernel_regularizer=spt.layers.l2_regularizer(config.l2_reg)):
        h_x = tf.to_float(x)
        h_x = spt.layers.resnet_conv2d_block(h_x, 64, strides=2 if config.dataset == 'celebA' else 1,
                                             scope='level_0')  # output: (28, 28, 16)
        h_x = spt.layers.resnet_conv2d_block(h_x, 64, scope='level_1')  # output: (14, 14, 32)
        # h_x = spt.layers.resnet_conv2d_block(h_x, 64, strides=2, scope='level_2')  # output: (14, 14, 32)
        h_x = spt.layers.resnet_conv2d_block(h_x, 96, strides=2, scope='level_3')  # output: (14, 14, 32)
        h_x = spt.layers.resnet_conv2d_block(h_x, 96, scope='level_4')  # output: (7, 7, 64)
        h_x = spt.layers.resnet_conv2d_block(h_x, 96, strides=2, scope='level_5')  # output: (7, 7, 64)
...
def main():
    ...
    beta = tf.Variable(initial_value=-2.0, dtype=tf.float32, name='beta', trainable=True)
