...
def q_net(x, posterior_flow, observed=None, n_z=None):
    ...
    with arg_scope([spt.layers.resnet_conv2d_block],
        ...
        h_x = spt.layers.resnet_conv2d_block(h_x, 128, strides=2, scope='mean_level_6')  # output: (7, 7, 64)
        h_x = spt.layers.resnet_conv2d_block(h_x, 256, strides=2, scope='mean_level_8')  # output: (7, 7, 64)
    ...
    with arg_scope([spt.layers.resnet_conv2d_block],
        ...
        h_x = spt.layers.resnet_conv2d_block(h_x, 128, strides=2, scope='std_level_6')  # output: (7, 7, 64)
        h_x = spt.layers.resnet_conv2d_block(h_x, 256, strides=2, scope='std_level_8')  # output: (7, 7, 64)
        final_channel = config.z_dim // (config.x_shape[0] // 8) // (config.x_shape[1] // 8)
...
def get_all_loss(q_net, p_net):
    with tf.name_scope('adv_prior_loss'):
        ...
        print('Shape of get_all_loss:')
        print(q_net['z'].log_prob())
        print(p_net['z'].log_prob())
        print(log_px_z)
