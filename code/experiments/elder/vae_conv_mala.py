...
def adv_prior_loss(q_net, pz_net, pw_net):
    ...
    return adv_psi_loss, adv_theta_loss, adv_phi_loss, adv_omega_loss
...
def compute_partition_function(train_flow, input_x, q_net, pz_net, pw_net):
    ...
    kl = np.mean(np.asarray(kl)) + log_Z
    # log_Z_var = np.mean(np.asarray(log_Z_var))

    print('log_Z=', log_Z, np.sqrt(log_Z_var))
    print('kl(q_phi(z), p_psi(z))=', kl)
    get_log_Z().set(log_Z)
...
def main():
    ...
    with tf.name_scope('optimizing'):
        ...
        print("========omega=========")
        print(omega_params)
        phi_optimizer = tf.train.AdamOptimizer(learning_rate_adv, 0.5, 0.9)
        psi_optimizer = tf.train.AdamOptimizer(learning_rate_adv, 0.5, 0.9)
        theta_optimizer = tf.train.AdamOptimizer(learning_rate)
        omega_optimizer = tf.train.AdamOptimizer(learning_rate_adv, 0.5, 0.9)
    ...
    with spt.utils.create_session().as_default() as session, \
        ...
        with spt.TrainLoop(tf.trainable_variables(),
            ...
            for epoch in epoch_iterator:
                ...
                if epoch % config.lr_adv_anneal_epoch_freq == 0:
                    learning_rate_adv.anneal()
