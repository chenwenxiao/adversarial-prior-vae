...
def q_net(x, observed=None, n_z=None, is_training=False, is_initializing=False):
    ...
    with arg_scope([spt.layers.conv2d],
        ...
        h_x = tf.to_float(x)
        h_x = spt.layers.conv2d(h_x, 32, scope='level_0')  # output: (16, 28, 28)
        h_x = spt.layers.conv2d(h_x, 32, strides=2, scope='level_1')  # output: (32, 14, 14)
        h_x = spt.layers.conv2d(h_x, 64, scope='level_2')  # output: (32, 14, 14)
        h_x = spt.layers.conv2d(h_x, 64, strides=2, scope='level_3')  # output: (64, 7, 7)
        h_x = spt.layers.conv2d(h_x, 256, scope='level_4', kernel_size=7)  # output: (64, 7, 7)

    # sample z ~ q(z|x)
    h_x = spt.ops.reshape_tail(h_x, ndims=3, shape=[-1])
    z_mean = spt.layers.dense(h_x, config.z_dim, scope='z_mean', kernel_initializer=tf.zeros_initializer())
    z_std = spt.layers.dense(h_x, config.z_dim, scope='z_logstd', kernel_initializer=tf.zeros_initializer(),
                             activation_fn=tf.nn.softplus)
    z = net.add('z', spt.Normal(mean=z_mean, logstd=spt.ops.maybe_clip_value(z_std, min_val=config.epsilon)),
...
def p_net(observed=None, n_z=None, gaussian_prior=False, mcmc_on_z=False,
    ...
    with arg_scope([spt.layers.deconv2d],
        ...
        h_z = spt.layers.deconv2d(h_z, 64, kernel_size=7, scope='level_1')  # output: (64, 7, 7)
        h_z = spt.layers.deconv2d(h_z, 64, scope='level_2')  # output: (64, 7, 7)
        h_z = spt.layers.deconv2d(h_z, 64, strides=2, scope='level_3')  # output: (64, 14, 14)
        h_z = spt.layers.deconv2d(h_z, 32, scope='level_4')  # output: (32, 14, 14)
        h_z = spt.layers.deconv2d(h_z, 32, strides=2, scope='level_4')  # output: (16, 28, 28)
        h_z = spt.layers.deconv2d(h_z, 32, scope='level_4')  # output: (16, 28, 28)
...
@add_arg_scope
@spt.global_reuse
def R_gamma(x, z, is_training=False, is_initializing=False):
    if config.act_norm:
        normalizer_fn = functools.partial(
            spt.layers.act_norm, initializing=is_initializing)
    else:
    ...
    with arg_scope([spt.layers.dense],
        ...
        with arg_scope([spt.layers.conv2d],
                       ...
                       channels_last=config.channels_last):
            h_x = tf.to_float(x)
            h_x = spt.layers.conv2d(h_x, 16, strides=2, scope='level_1')  # output: (16, 14, 14)
            h_x = spt.layers.conv2d(h_x, 32, strides=2, scope='level_2')  # output: (32, 7, 7)
            h_x = spt.layers.conv2d(h_x, 64, strides=2, scope='level_3')  # output: (64, 4, 4)
            h_x = spt.layers.conv2d(h_x, 128, strides=2, scope='level_4')  # output: (128, 2, 2)
        h_x = tf.layers.flatten(h_x)

        h_r = spt.ops.broadcast_concat(tf.to_float(z), tf.to_float(h_x), axis=-1)
        h_r = spt.layers.dense(h_r, 500, scope='level_5')
        h_r = spt.layers.dense(h_r, 500, scope='level_6')
        h_r = spt.layers.dense(h_r, 200, scope='level_7')
        h_r = spt.layers.dense(h_r, 200, scope='level_8')
        h_r = spt.layers.dense(h_r, 200, scope='level_9')
        h_r = spt.layers.dense(h_r, 200, scope='level_10')
    h_r = spt.layers.dense(h_r, 1, scope='level_11')
                    ...
                    while step_iterator.has_next:
                        ...
                        for step, [x] in step_iterator:
                            ...
                            [_, batch_gamma_loss, _z, _w] = session.run(
                                [gamma_train_op, adv_gamma_loss, debug_energy_z, debug_energy_Gw], feed_dict={
                                    input_x: x
                                })
                            loop.collect_metrics(energy_z=_z)
                            loop.collect_metrics(energy_Gw=_w)
