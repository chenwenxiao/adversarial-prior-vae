...
\begin{align*}\label{eq:direct_2nd_optimization}
	...
	\tag{10}
\end{align*}
We apply an assumptions $\E_{p_\theta(x|z)} D(x) = D(G(z))$ and an approximation $D$ to simplify it. The $D$ in \cref{eq:direct_2nd_optimization} could be approximated by $D$ obtained by $W^1(p^\dag, p^*)$ (it is independent on $\lambda$ and feasible), if we restrict $p_\lambda$ near the $p_\mathcal{N}$, as \cref{subsec:naive_vaepp} and \cref{subsec:improve_of_vaepp} does. The simplied optimization is:
