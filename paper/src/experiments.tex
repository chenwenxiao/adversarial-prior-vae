...
It is important to notice that the GAN in VAEPP only plays the role that generates $z_0$ with high $p_\lambda(z_0)$, in latent space with small dimension, instead of image. The ability of VAEPP that generates images from $z$ is depend on the decoder. 
\begin{figure}[tb]
	\centering
