...
Accept-Reject Sampling (ARS)~\cite{bauer2019resampled} is useless for $p_\lambda$ since ARS requires that $p_\lambda(z) / p_\mathcal{N}(z)$ is bounded by a constant $M$ for any $z$ on the support of $p_\lambda$. But it is hard to ensure that there exists an enough small $M$ in VAEPP.
